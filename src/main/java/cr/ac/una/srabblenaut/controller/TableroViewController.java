...
public void rellenarmazoJug(){
    ...
    for (int i = 0; i < 6; i++) {
        Mazojug1[i]= bolsa[i];
        ImageView img2 = Mazojug1[i];
       img2.setImage(Mazojug1[i].getImage());
       img2.setFitHeight(22);
       img2.setFitWidth(22);
        
    hBoxLosetasJugador1.getChildren().add(img2);
//    hBoxLosetasJugador2.getChildren().add(vboxTablero);
//    hBoxLosetasJugador3.getChildren().add(vboxTablero);
//    hBoxLosetasJugador4.getChildren().add(vboxTablero);   
    ...
    public void rellenarHBoxBolsa(){
        ...
        if (hBoxbolsaFichas.getChildren().isEmpty()){
        
            //Funcionalidad de drag and drop de las fichas.
            ImageView img2 = bolsa[0];
            Image image = new Image(bolsa[0].getImagen());
            img2.setImage(image);
           img2.setOnDragDetected((MouseEvent event) -> {
...
event.consume();
            ...
            img2.setOnDragDone((DragEvent event) -> {
                /* the drag-and-drop gesture ended */
//                System.out.println("onDragDone");
/* if the data was successfully moved, clear it */
if (event.getTransferMode() == TransferMode.MOVE) {
    img2.setAccessibleText("Eliminada");
    moverFichasMezalogia((Fichas)img2);
    hBoxbolsaFichas.getChildren().remove(img2);
     rellenarHBoxBolsa();
}
...
event.consume();
            ...
            img2.setFitWidth(70);
             hBoxbolsaFichas.getChildren().add(img2);  
